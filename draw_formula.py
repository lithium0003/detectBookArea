...
if len(sys.argv) != 2:
    print("Usage: python draw_formula.py <target_file>")
    sys.exit(1)
...
if os.path.exists(target_file + '.formula.png'):
    labelim0 = Image.open(target_file + '.formula.png').convert('L')
...
def on_press(event):
    ...
    if event.key == ' ':
        print("Saved.")
        Image.fromarray(labelim).save(target_file + '.formula.png')
        return
