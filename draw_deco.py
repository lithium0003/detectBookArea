...
if len(sys.argv) != 2:
    print("Usage: python draw_deco.py <target_file>")
    sys.exit(1)
...
if os.path.exists(target_file + '.decorate.png'):
    rimline0 = Image.open(target_file + '.decorate.png').convert('L')
    rim_map = np.array(rimline0) > 0
else:
    rim_map = np.zeros(im_y.shape, dtype=bool)
prev_rim_map = rim_map

fig = plt.figure("(Decorate) Space: save, u: undo")
...
def on_press(event):
    ...
    if event.key == ' ':
        print("Saved.")
        Image.fromarray(rim_map).convert('L').save(target_file + '.decorate.png')
        return
